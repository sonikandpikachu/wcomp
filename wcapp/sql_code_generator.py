...
import re
code2 = 'DROP TABLE IF EXISTS `wComp`.`wComp_ColorList` DROP TABLE IF EXISTS `wComp`.`wComp_ColorList`'
searches = re.findall('DROP TABLE IF EXISTS `wComp`.`.*`', code)

for s in searches:
    table = s.split('.')[1].strip('`')
    classcode = 'class ' + table + '(Base): \n\t' + '__table__ = Table("' + table + '", metadata, autoload=True) \n\t' 
    print classcode
